...
orders.forEach((order) => {
  if (order.details) {
    ...
    const result =
      customerTotalPrice.length > 0
        ? customerTotalPrice.some((q) => q.id === order.customerId)
        : null;
    if (result) {
      customerTotalPrice.find((q) => q.id === order.customerId).totalPrice =
...
bestCustomer = customerTotalPrice.sort(
  (a, b) => b.totalPrice - a.totalPrice
)[0];

worstCustomer = customerTotalPrice.sort(
  (b, a) => b.totalPrice - a.totalPrice
)[0];
...
//1998 yılı mayıs ayında verilen siparişler

// const filteredOrders = orders.filter(
//   (q) =>
//     new Date(q.orderDate).getFullYear() === 1998 &&
//     new Date(q.orderDate).getMonth() === 4
// );
// console.log(filteredOrders);

// mayıs 98 olanların verdikleri siparişlerin toplam tutarı nedir?
...
orders.forEach((order) => {
  if (order.details) {
    if (
      new Date(order.orderDate).getFullYear() === 1998 &&
