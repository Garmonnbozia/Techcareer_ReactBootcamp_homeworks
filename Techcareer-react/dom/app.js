...
const header =
  "<tr> <th>ID</th> <th>Name</th> <th>Price</th> <th>KDVPrice</th>";
...
products.forEach((product) => {
  if (product.unitsInStock <= 0) {
    tableElement.innerHTML += `<tr style=color:red;><td>${
      product.id
    }</td> <td>${product.name}</td> <td>${product.unitPrice}</td> <td>${(
      product.unitPrice * 1.18
    ).toFixed(2)}</td> </tr>`;
  } else {
    tableElement.innerHTML += `<tr><td>${product.id}</td> <td>${
      product.name
    }</td> <td>${product.unitPrice}</td> <td>${(
      product.unitPrice * 1.18
    ).toFixed(2)}</td> </tr>`;
  }
});
