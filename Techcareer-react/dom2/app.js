...
const tableContent = `<tr> <th>CustomerId</th> <th>shipName</th> <th>city</th> <th>orderDate</th> <th>TotalPrice</th> </tr>`;

elementTable.innerHTML = tableContent;

console.log(elementUl);
console.log(elementTable);

orders.forEach((element) => {
  if (element.shipVia === 1) {
    const elementLi = document.createElement("li");
    elementLi.innerHTML += `${element.id}`;
    elementUl.appendChild(elementLi);
    elementLi.style = "color: red;";
  }
  if (element.shipVia === 2) {
    const elementLi = document.createElement("li");
    elementLi.innerHTML += `${element.id}`;
    elementUl.appendChild(elementLi);
    elementLi.style = "font-weight: bold;";
  } else {
    const elementLi = document.createElement("li");
    elementLi.innerHTML += `${element.id}`;
    elementUl.appendChild(elementLi);
...
orders.forEach((e) => {
  let dateObj = new Date(e.orderDate);
  let myDate = dateObj.getFullYear() + "/" + (dateObj.getMonth() + 1);

  e.details.forEach((detail) => {
    priceOfUnit = `${detail.unitPrice}`;
    quantityOfUnit = `${detail.quantity}`;
    totalPrice = (priceOfUnit * quantityOfUnit).toFixed(2);
    console.log(totalPrice);
  });
  elementTable.innerHTML += `<tr><td>${e.id}</td> <td>${e.shipName}</td> <td>${e.shipAddress.city}</td> <td>${myDate}</td> <td>${totalPrice}</td> </tr>`;
});
